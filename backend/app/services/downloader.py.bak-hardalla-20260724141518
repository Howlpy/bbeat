...
def _fallback_candidate_matches(e: dict, artist_norm: str, title_words: set[str]) -> bool:
    """Filtra resultados de búsquedas amplias para no bajar otra canción."""
    cand_title = _norm_text(e.get("title") or "")
    uploader = _norm_text(e.get("uploader") or e.get("channel") or "")
    cand_words = set(cand_title.split())
    ...
    return overlap >= 0.5 and artist_matches
...
def download_with_ytdlp(
    ...
    searches = [
        ("exacta", 8, meta.search_query, False),
        ("ampliada", 30, f"{meta.primary_artist} canciones {meta.title}", True),
        ("por artista", 30, meta.primary_artist, True),
    ]
    for label, limit, query, filter_broad in searches:
        try:
        ...
        if filter_broad:
            found = [
                e for e in found
                if _fallback_candidate_matches(e, artist_norm, title_words)
            ]
        if found:
            entries = found
            if label != "exacta":
                log.info("fallback %s: %d candidatos válidos · %s", label, len(found), query)
            break
