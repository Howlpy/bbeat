...
def getSongsByGenre(params, user, session) -> dict:
    # Muchos clientes (incluido Substreamer) usan este endpoint para poblar
    # la vista "Canciones". No todas las pistas tienen álbum: los singles
    # descargados desde YouTube se guardan sin album_id, pero deben aparecer
    # igualmente en el catálogo completo.
    ...
    # Bbeat no tiene un campo genre en Track; una petición sin género (la
    # forma que usan los clientes para "todas") debe devolver el catálogo
    # completo, no una respuesta vacía.
    if genre:
